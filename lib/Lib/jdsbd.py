...
class Model:
    ...
    def __init__(self, path):
        ...
        self.path = path
    def set_svm_model_file(self, svm_model_file_path, svm_light_classifier):
        self.svm_model_file = svm_model_file_path
        self.svm_classify = svm_light_classifier
...
class SVM_Model(Model):
    ...
    def train(self, doc):
        ...
        svm_model_file = '%ssvm_model' %self.path
        ...
        options = '-c 1 -v 0'
        cmd = '%s %s %s %s' %(SVM_LEARN, options, train_file, svm_model_file)
        os.system(cmd)
        sys.stderr.write('done!\n')
    ...
    def classify(self, doc, verbose=False):
        ...
        print 'degug:' + self.svm_model_file
        svm_model_file = self.svm_model_file
        SVM_CLASSIFY = self.svm_classify
        #svm_model_file = '%ssvm_model' %self.path
        if not self.feats: sbd_util.die('Incomplete model')
        if not os.path.isfile(svm_model_file): sbd_util.die('no model [%s]' %svm_model_file)
        ...
        options = '-v 0'
        cmd = '%s %s %s %s %s' %(SVM_CLASSIFY, options, test_file, svm_model_file, pred_file)
        print cmd
        os.system(cmd)
...
def build_model(files, options):
    ...
    model.save()
    return model
def load_sbd_model_from_file(model_path = 'model_nb/', use_svm=False, atrFile='', lwdFile='' ,nbvFile=''):
    sys.stderr.write('loading model from [%s]... ' %model_path)
    if use_svm: model = SVM_Model(model_path)
    else: model = NB_Model(model_path)
    model.load_from_file(atrFile, lwdFile, nbvFile)
    sys.stderr.write('done!\n')
    return model
