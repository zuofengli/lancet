...
/**
 * <p>
 * The Trainer class will train a CRF to extract entities from a customized
 * dataset. The input file must be tokenized with one sentence per line, with a
 * "|" (vertical pipe) separating a word/token from its label. The first token
 * of an entity name should have a label beginning with "B-", all other entity
 * token labels should begin with "I-", and non-entity tokens should be labeled
 * with "O":
 * 
 * <pre>
...
public class Trainer {
	...
	private static String CAPS = "[A-Zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½]";
	private static String LOW = "[a-zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½]";
	private static String CAPSNUM = "[A-Zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½0-9]";
	private static String ALPHA = "[A-Zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½a-zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½]";
	private static String ALPHANUM = "[A-Zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½a-zï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½0-9]";
	...
	/**
	 * <p>
	 * Takes input <tt>trainFile</tt> (format described above), and saves a
	 * trained linear-chain CRF on the data using ABNER's default feature set in
	 * the corresponding output <tt>modelFile</tt>.
	 * 
	 * <p>
	 * <i>Warning: training will take several hours, perhaps even days to
	 * complete depending on corpus size and number of entity tags.</i>
	 */
	 ...
	 * "PROTEIN", "DNA", etc.) allows the model to periodically output progress
	 * in terms of precision/recall/f1 during training. <i>Note: do not use "B-"
	 * or "I-" prefixes.</i>
	 */
		...
		try {
			...
			p = new SerialPipes(
					new Pipe[] {
							...
							new RegexMatches("INITCAPS", Pattern
									.compile("[A-Z].*")),
							new RegexMatches("INITCAPSALPHA", Pattern
									.compile("[A-Z][a-z].*")),
							new RegexMatches("ALLCAPS", Pattern
									.compile("[A-Z]+")),
							new RegexMatches("CAPSMIX", Pattern
									.compile("[A-Za-z]+")),
							new RegexMatches("HASDIGIT", Pattern
									.compile(".*[0-9].*")),
							new RegexMatches("SINGLEDIGIT", Pattern
									.compile("[0-9]")),
							new RegexMatches("DOUBLEDIGIT", Pattern
									.compile("[0-9][0-9]")),
							new RegexMatches("NATURALNUMBER", Pattern
									.compile("[0-9]+")),
							new RegexMatches("REALNUMBER", Pattern
									.compile("[-0-9]+[.,]+[0-9.,]+")),
							new RegexMatches("HASDASH", Pattern
									.compile(".*-.*")),
...
//
							...
							new OffsetConjunctions(
									new int[][] { {-6}, {-5}, {-4}, {-3}, {-2}, { -1 }, { 1 } , {2}, {3}, {4}, {5}, {6}}),
							
//							new OffsetConjunctions(
//											new int[][] {{ 1 }, { 2 },{3 }}),

							new RegexMatches("ALPHANUMERIC", Pattern
									.compile(".*[A-Za-z].*[0-9].*")),
							new RegexMatches("ALPHANUMERIC", Pattern
									.compile(".*[0-9].*[A-Za-z].*")),

							new RegexMatches("ROMAN", Pattern
									.compile("[IVXDLCM]+")),
							new RegexMatches("HASROMAN", Pattern
									.compile(".*\\b[IVXDLCM]+\\b.*")),
							new RegexMatches("GREEK", Pattern.compile(GREEK)),
							new RegexMatches("HASGREEK", Pattern
									.compile(".*\\b" + GREEK + "\\b.*")),

							new RegexMatches("PUNCTUATION", Pattern
									.compile("[,.;:?!-+]$")),
							/**
//							 * zuofeng's code
//							 */
////							ntg 1/150 ( nitroglycerin 1/150 ( 0.4 mg ) )		
//							new RegexMatches("OPENPAREN=", Pattern
//									.compile("\\(")),
//							new RegexMatches("CLOSEPAREN=", Pattern
//									.compile("\\)")),
//									
//							new RegexMatches("BACKSLASH=", Pattern
//									.compile("/")),
//							
////						    q5min x 3 prn
//							new RegexMatches("MULTIPLEPLUS", Pattern
//									.compile("x", Pattern.CASE_INSENSITIVE)),
//							
//							new RegexMatches("PRN", Pattern
//											.compile("^prn$", Pattern.CASE_INSENSITIVE)),
////													
////							new RegexMatches("PERCENT", Pattern
////									.compile("%")),
//							/**
//							 * zuofeng's code end
//							 */
							// new PrintTokenSequenceFeatures(), // for
							// debugging
...
//			for(int i= 0; i< 2; i++){
//				trainFile = "./lsdb/lsdb" + i + ".train";
				
				System.out.println("Reading '" + trainFile + "' file...");
				
//				its.resetLineIndex();
				trainingData.add(new LineGroupIterator(new FileReader(new File(
						trainFile)), Pattern.compile("^.*$"), false));
//			}
...
//			crf.addStatesForHalfLabelsConnectedAsIn(trainingData);
			...
			if (tags != null) {
				...
				MultiSegmentationEvaluator eval = new MultiSegmentationEvaluator(
						bTags, iTags, false);
				crf.train(trainingData, (InstanceList) null,
						(InstanceList) null, eval, 99999, 10, new double[] {
								.2, .5, .8 });
			}
			...
			// otherwise, just get going...
				...
				crf.train(trainingData, (InstanceList) null,
						(InstanceList) null, (MultiSegmentationEvaluator) null,
						99999, 10, new double[] { .2, .5, .8 });
			}
