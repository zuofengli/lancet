...
public class SimpleTagger
{
      ...
      for (int l = 0; l < tokens.length; l++) {
        ...
        for (int f = 0; f < nFeatures; f++)
          featureIndices[f] = features.lookupIndex(tokens[l][f]);
       	fvs[l] = featureInductionOption.value ? new AugmentableFeatureVector(features, featureIndices, null, featureIndices.length) : 
  ...
  public static CRF train2(InstanceList training, TransducerEvaluator evaluator, double var){
	  return SimpleTagger.train(training, (InstanceList)null,
			  evaluator, new int[] {0,1},
		      "O",
		      "O,I-*", null,
		      true, 99999, var, (CRF)null);
  }
  /**
  ...
  {
    Pattern forbiddenPat = null;
    if (forbidden != null)
    	Pattern.compile(forbidden);
   
    Pattern allowedPat = null;
    if (allowed != null)
